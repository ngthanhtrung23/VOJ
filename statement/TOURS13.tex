...
\begin{itemize}
	...
	\item Dòng thứ \emph{ j } trong số m dòng tiếp theo chứa ba số nguyên duong $u_{j}$ , $v_{j}$ , c($u_{j}$ , $v_{j}$ ) cho biết thông tin về tuyến đường thứ j. Giả thiết là $u_{j}$ ≠ $v_{j}$ ; c($u_{j}$ , $v_{j}$ ) $<$ 10^6; \emph{ j } = 1, 2, …, m
\end{itemize}
